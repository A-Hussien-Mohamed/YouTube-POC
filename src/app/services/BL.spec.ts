...
describe("DataService", () => {
    ...
    let apiService: ConfigService;
    beforeEach(() => {
        service = new DataService(apiService);
    });

    it("should set the current video list ", () => {
        let searchResponse = {
        ...
        service.setVideoList(searchResponse);
        let subscribOncurrentList: response;
        service.currentVideoList.subscribe(result => subscribOncurrentList = result);
        expect(subscribOncurrentList.items.length).toBeGreaterThanOrEqual(1);
    });

    it("should remove the current video list ", () => {
        let searchResponse = {
            "kind": "youtube#searchListResponse",
            "etag": "\"XpPGQXPnxQJhLgs6enD_n8JR4Qk/yzBm4B-ug_IMyT4sI2H8VTQNN8w\"",
            "nextPageToken": "CAoQAA",
        ...
        service.setVideoList(searchResponse);
        let subscribOncurrentList: response;
        service.deleteCurrentVideoList();
        service.currentVideoList.subscribe(result => subscribOncurrentList = result);

        expect(subscribOncurrentList.items).toBeUndefined();
    });

    it("should fail if search predicate id is undefined on getting channel details ", () => {
        let predicate: searchPridicate = new searchPridicate;
        predicate.maxResults = '10';
        predicate.query = 'Spongebob';
        service.setChannelDetails(null);
        let result;
        service.currentChannel.subscribe(r => { result = r.items })
        expect(result).toBeUndefined();
    });

    it("should get the latest update of the search predicate", () => {
        let predicate: searchPridicate = new searchPridicate;
        predicate.maxResults = '10';
        predicate.query = 'Spongebob';
        service.setSearchPridicate(predicate);
        let resultPredicate: searchPridicate;
        service.currentPredicate.subscribe(r => resultPredicate = r);
        expect(resultPredicate).toEqual(predicate);
    });
});